...
const handleCheckbox =(e) => {
  ...
  return (
    <>
      ...
      <h1 className="font-bold text-center text-3xl ">List Manager-Manage Your Tasks </h1>
          <div className="addTodo my-5 flex flex-col gap-4">
          <h2 className="text-2xl font-bold">Add a Todo</h2>
          <div className="flex">
          <input onChange={handleChange}  value={todo} type="text" className='w-full rounded-full px-5 py-1' />
          <button onClick={handleAdd} disabled={todo.length<=3} className="bg-violet-800 hover:bg-violet-950 disabled:bg-violet-700  p-4 py-2 text-sm font-bold text-white rounded-full mx-2">
           Save
          </button>
          </div>
        </div>
        ...
        <div className="todos">
        {todos.length===0 && <div className='m-5'>No Todos To </div>}
        {todos.map(item=>{
